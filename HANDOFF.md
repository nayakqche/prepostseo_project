...
- Repo: `nayakqche/prepostseo_project`
- Default branch: `main`
- Working branch: `claude/fitseo-site-replica-uav1ql`
...
3. If the last PR on the branch is already merged, restart the branch from the latest `main` before starting new work:
   `git fetch origin main && git checkout -B claude/fitseo-site-replica-uav1ql origin/main`
...
| #3 | This handoff | Added `HANDOFF.md` |
...
```bash
git clone https://github.com/nayakqche/prepostseo_project.git
cd prepostseo_project
npm install
...
- Theme tokens (in globals.css): `bg-background`, `bg-surface`, `border-border`, `text-primary`, `text-muted`, `text-success`, `text-danger`, `text-accent`, `gradient-bg`
...
- Primary button: `px-6 py-2.5 gradient-bg text-white rounded-lg text-sm font-medium hover:opacity-90 transition-opacity disabled:opacity-50`

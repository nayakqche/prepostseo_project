    ...
    <div>
      ...
      <div className="flex items-center gap-3 mt-4">
        <button
          ...
          className="px-6 py-2.5 gradient-bg text-white rounded-lg text-sm font-medium hover:opacity-90 transition-opacity disabled:opacity-50"
        >
              ...
              <div className="space-y-2">
                {parsed.errors.map((err, i) => (
                  <div key={i} className="flex items-start gap-3 p-3 bg-background border border-border rounded-xl text-sm">
                    <span className={`px-2 py-0.5 text-xs rounded-full font-medium ${typeColors[err.type] || "bg-gray-100 text-gray-600"}`}>
                      {err.type}
                    </span>
                    <div>
